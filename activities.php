...
<div class="container">
    ...
    <div class="page-header">
        <h1 class="page-title" style="background: linear-gradient(135deg, #7c3aed, #8b5cf6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; background-clip: text;">
            Activités à Agen
        </h1>
        <p class="page-subtitle">
            Explorez Agen à travers ses musées, parcs, monuments historiques et activités de loisirs.
        </p>
    ...
    <!-- Results Container -->
    <div id="results-container" class="results-container" data-type="activities">
        <div class="loading">
...
async function loadActivities() {
    ...
    try {
        ...
        if (data.success) {
            countSpan.textContent = `${data.count} activité${data.count > 1 ? 's' : ''} trouvée${data.count > 1 ? 's' : ''}`;
            ...
            if (data.items.length === 0) {
                ...
                const itemsHtml = data.items.map(createActivityCard).join('');
        ...
        `;
        countSpan.textContent = '0 activité trouvée';
    }
}
...
function createActivityCard(activity) {
    ...
    return `
        <div class="card hover-scale" data-id="${activity.id}" data-type="activity">
            <div class="card-image">
                ...
                <button class="favorite-btn" data-id="${activity.id}" data-type="activity">
                    <svg width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
            ...
            <div class="card-content">
                ...
                <div class="card-category">${activity.category} • ${activity.type_original}</div>
                ...
                <a href="detail.php?type=activity&id=${activity.id}" class="btn" style="background: linear-gradient(135deg, #7c3aed, #8b5cf6); color: white;">
                    Voir les détails
                </a>
            </div>
        </div>
